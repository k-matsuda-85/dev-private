...
var Tool_Help = {
    ...
    Add: function () {
        // 二重登録チェック
        if (Tool_Help.Enabled) {
            return;
        }
        Tool_Help.Enabled = true;

        // 要素を作成
        $("#ToolArea-View").append($("<div>").attr("id", "Tool-Help").addClass("Tool-Common-SizeA Tool-Help-OFF"));
    ...
    Command: function () {
        // パラメータチェック
        if ($("#ViewerConfig").data("HelpUrl") == "") {
            return;
        }

        // Viewer起動処理
        Common_WindowOpen($("#ViewerConfig").data("HelpUrl"), "Help", true);
    }
}
