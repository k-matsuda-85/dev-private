...
var Tool_CTAnalyze = {
    ...
    Add: function () {
        // 二重登録チェック
        if (Tool_CTAnalyze.Enabled) {
            return;
        }
        Tool_CTAnalyze.Enabled = true;

        // 要素を作成
        $("#ToolArea-View").append($("<div>").attr("id", "Tool-CTAnalyze").addClass("Tool-Common-SizeA Tool-CTAnalyze-OFF"));
    ...
    Click: function () {
        // 未選択状態の場合
        if ($("#Tool-CTAnalyze").hasClass("Tool-CTAnalyze-OFF")) {
            // ツール名更新
            if (Tool_Menu.UpdateName("CTAnalyze")) {
                // メニューキャンセル処理
                Tool_Menu.Cancel();
    ...
    Command: function (command) {
        ...
        if (command == true) {
            ...
            // アイコン変更
            $("#Tool-CTAnalyze").removeClass("Tool-CTAnalyze-OFF").addClass("Tool-CTAnalyze-ON");
        }
        else {
            // ViewerCtrl呼び出し
            ...
            // アイコン変更
            $("#Tool-CTAnalyze").removeClass("Tool-CTAnalyze-ON").addClass("Tool-CTAnalyze-OFF");
        }
    }
}
