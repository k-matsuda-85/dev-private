...
// シリーズ表示変更
var Tool_SeriesPanelChange = {
    // 有効
    Enabled: false,
    // 追加
    Add: function () {
        // 二重登録チェック
        if (Tool_SeriesPanelChange.Enabled) {
            return;
        }
        Tool_SeriesPanelChange.Enabled = true;

        // 要素を作成
        $("#ToolArea-View").append($("<div>").attr("id", "Tool-SeriesPanelChange").addClass("Tool-Common-SizeA Tool-SeriesPanelChange-OFF"));

        // ボタンクリックイベント設定
        $("#Tool-SeriesPanelChange").on({
            // タッチ開始イベント設定
            "touchstart": function (e) {
                ...
                var event = function (e) {
                    ...
                    if (e.type == "touchend" && !pointInfo.IsDrag) {
                        // コマンド処理
                        Tool_SeriesPanelChange.Command();
                    }
    ...
    Command: function () {
        // 表示シリーズ取得
        var $this = $("#SeriesList .SeriesList-Group:visible");
        if ($this.length == 0) {
            return;
        }
        ...
        // 表示方法をトグル
        var type;
        if ($this.children(":first").hasClass("SeriesList-Item1-Content")) {
            type = 0;
        }
        else {
            type = 1;
        ...
        // シリーズ一覧表示変更処理
        ViewerSeriesList_SeriesPanel_Change($this.data("StudyKey"), type);
    }
}
