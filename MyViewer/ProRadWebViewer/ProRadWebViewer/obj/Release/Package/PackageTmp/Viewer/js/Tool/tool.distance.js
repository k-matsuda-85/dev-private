...
var Tool_Distance = {
    ...
    Add: function () {
        // 二重登録チェック
        if (Tool_Distance.Enabled) {
            return;
        }
        Tool_Distance.Enabled = true;

        // 要素を作成
        $("#ToolArea-View").append($("<div>").attr("id", "Tool-Distance").addClass("Tool-Common-SizeA Tool-Distance-OFF"));
    ...
    Click: function () {
        // 未選択状態の場合
        if ($("#Tool-Distance").hasClass("Tool-Distance-OFF")) {
            // ツール名更新
            if (Tool_Menu.UpdateName("Distance")) {
                // メニューキャンセル処理
                Tool_Menu.Cancel();
    ...
    Command: function (command) {
        ...
        if (command == true) {
            ...
            // アイコン変更
            $("#Tool-Distance").removeClass("Tool-Distance-OFF").addClass("Tool-Distance-ON");
        }
        else {
            // ViewerCtrl呼び出し
            ...
            // アイコン変更
            $("#Tool-Distance").removeClass("Tool-Distance-ON").addClass("Tool-Distance-OFF");
        }
    }
}
